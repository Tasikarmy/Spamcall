...
class prankcall{
	...
	public function run(){
		while(true){
			echo "root@RandiSr=# Kirim Mode Manual/Tidak ?(y/t) : ";
			$loop = $this->get();
			if($loop=="y" OR $loop=="t"){
				break;
			}else{
		...
		if($loop=="y"){
			echo "root@RandiSr=# Masukan Jumlah Spam #-> ";
			$many = $this->get();
			$this->loop($many);
		}else{
			$this->ekse();
...
echo $blue."
[!] Author   : RandiSr		              [!]
[!] Youtube  : www.youtube.com/RANDI OLOYY  [!]
[!] github   : ??                         [!]\n";
echo "\n";
echo "\n";
echo "==============================================\n";
...
echo " \n";
sleep ('0.4');
echo "root@.RandiSr=# Masukan No Hp Target #-> ";
$no = get();
$n = new prankCall($no);
$n->run();